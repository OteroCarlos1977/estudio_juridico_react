...
function todayISO(now = new Date(), timeZone = BUSINESS_TIME_ZONE) {
  return formatDateParts(now, timeZone);
}

function currentMonthISO(now = new Date(), timeZone = BUSINESS_TIME_ZONE) {
  return todayISO(now, timeZone).slice(0, 7);
}

function currentTimestampSQL(now = new Date(), timeZone = BUSINESS_TIME_ZONE) {
...
function isPastISODate(value, now = new Date(), timeZone = BUSINESS_TIME_ZONE) {
...
function formatDisplayDate(value, locale = "es-AR") {
  if (!isValidISODate(value)) {
    return "";
  }
